...
Codex Trajectory is a privacy-aware Codex plugin whose MCP tools turn local task logs into an event ledger and interactive timeline. It shows turns, approximate model steps, reasoning summaries, assistant messages, tool timing, subagents, compaction, token usage, and failures without changing the original logs. Its optional live stop controls send a user-authorized stop action to the current Codex task; the configured Browser path pauses an active Goal before interrupting its turn, and neither path deletes worktrees or modifies task files.
...
Click **Live window** to keep the current task visible without launching a standalone app. Inside Codex, the component uses the host's supported `fullscreen` presentation to fill its side panel; a frozen header shows the whole task's cumulative Token breakdown and current turn/step/record cursor, while the remaining height is a separately scrollable safe-summary event stream. When Codex records account rate-limit windows, the header's upper-right quota badge shows the remaining percentage for each window—normally 5-hour and weekly—with reset times in its tooltip; the badge stays hidden when that data is unavailable. A one-click **Stop** control stops the current Codex task. An optional auto-stop guard, off by default with a 10% default threshold, stops at most once while the same displayed quota cycle remains at or below the configured remaining percentage. In the configured Browser view, the page binds directly to the requested task ID, pauses an active Goal through `thread/goal/set`, and then calls `turn/interrupt` for the exact active turn; if the separate state poll is temporarily unavailable, a threshold crossing still attempts this bounded stop sequence and retries transient failures with backoff. Stop/Idle and manual-stop availability follow the active App Server turn rather than the potentially delayed local event ledger. When that turn finishes, **Requested** changes to **Idle**, and a later running turn rearms manual Stop. As a fallback, the automatic latch deliberately survives later turns—including `/goal` continuations—until all displayed windows recover above the threshold, a quota window resets, or the user changes the guard configuration. Pausing the Goal prevents normal continuation; the cycle latch prevents a start/interrupt loop even if an older host lacks Goal RPCs. The guard configuration and successful quota-cycle latch use the host's widget state when available. In the standard Apps surface, where the host exposes only the follow-up bridge, Stop remains a model-mediated safe-shutdown request that asks the task to pause an active Goal when that control is available, stop related subagents, and preserve worktrees and uncommitted changes. Stop controls are disabled if the viewer is switched to a task other than the one that opened it, or when neither the host follow-up bridge nor the configured in-app Browser stop bridge is available.
...
### Optional Codex in-app Browser shortcut

The inline viewer has an off-by-default **In-app Browser shortcut** setting. When enabled, a dependency-free local watcher connects only to the selected `127.0.0.1` Chrome DevTools Protocol port, survives Codex React rerenders, and injects **View trajectory** immediately after **Full access**. The watcher serves the same complete trajectory interface from a tokenized random loopback port: task selection, statistics, Token details, timing overview, filters, event ledger, refresh, pagination, explicit full-detail confirmation, and interactive stop controls are shared with the MCP Apps resource. A user click opens it as a Codex in-app Browser tab for the selected task. It starts in safe-summary mode and does not cover the Environment/Changes surface; Codex owns the Browser tab and its normal side-panel switching controls. For newly created tasks, Codex may retain a `client-new-thread:*` sidebar key after App Server assigns the canonical task UUID; the shortcut resolves that UUID from the active conversation surface and remains disabled until it is available, so reads and stops never use the temporary key. The page continuously reads a fixed allowlist of Codex's computed background, surface, border, text, status, and accent colors, so Codex's System, Light, and Dark appearance choices resolve to the same effective palette in the already-open trajectory tab. Opening the page never changes the composer, posts a task message, invokes the model, or creates a new turn. A separate bounded read-only endpoint combines the current projected turn identifier with a lightweight App Server status read that explicitly omits turn history, keeping Stop/Idle controls current without repeatedly materializing very large tasks; only initial bootstrap without a candidate, or rebinding after App Server rejects a stale candidate, performs one full read. If the user clicks **Stop** or arms the quota guard, a fixed-intent endpoint checks `thread/goal/get`, changes an active Goal to `paused`, and only then sends the exact bound task/turn pair to `turn/interrupt`. It performs a lightweight history-free status read only when the interrupt fails or races with completion. If the task advanced to a newer turn, the wrapper invalidates the rejected candidate and performs that one-time bootstrap before a retry. If the Goal cannot be paused, the turn is not interrupted and the UI reports that error, avoiding the original continuation loop. If the turn completes during the interrupt round trip, the result becomes **Idle**; a still-running failure reports a bounded App Server timeout, stale-turn, or interrupt error. Arbitrary messages are not accepted, composer drafts and attachments are never touched. Disabling the setting removes the injected DOM, closes the local page server, and stops the watcher. Every updated MCP runtime start reconciles the persisted opt-in and safely replaces an older verified watcher instance, so cache-buster upgrades do not leave stale code serving the page. The setting, heartbeat, and watcher lock are read only as bounded single-link regular files under `CODEX_HOME/codex-trajectory`; task logs and theme values remain untouched.

CDP must already be enabled when the app starts. On macOS, completely quit ChatGPT/Codex and relaunch it from Terminal with:
