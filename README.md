...
On macOS and Windows, **View trajectory** is restored only after operating-system peer authentication succeeds for the exact token-bearing CDP connection. macOS resolves the connected TCP owner and requires the signed `com.openai.codex` application from OpenAI's signing team; Windows resolves the connection owner through the system TCP table and requires the authenticated `OpenAI.Codex` package family and process ancestry. The watcher discovers the desktop host that launched the plugin, rejects PID reuse, checks every new connection before its WebSocket handshake can carry a viewer token, and exits when that host identity disappears. Authentication failure is fail-closed: the local viewer URL/token never leaves the watcher and no token-bearing injection is used, while the separately bounded direct-stop path remains available after explicit opt-in. Platforms without this peer binding keep the Browser shortcut unavailable. After its one-time Codex hook review, the bundled asynchronous session-start bootstrap restores an already enabled watcher earlier in startup; Windows uses uv's windowless `uvw.exe` and keeps the watcher in that session-bound hook process so no console is created and no detached child must escape the Codex Job Object. The MCP runtime remains the fallback, and neither path enables CDP for the user. Updated runtimes use a cross-process control lock and settings revision check when replacing a verified older watcher, so a stale recovery cannot overwrite a later disable or port change. Heartbeat errors are reduced to a fixed public category rather than exposing exception text or local paths. Settings, heartbeat, and watcher locks remain bounded single-link regular files under `CODEX_HOME/codex-trajectory`; task logs, drafts, and attachments remain untouched.
