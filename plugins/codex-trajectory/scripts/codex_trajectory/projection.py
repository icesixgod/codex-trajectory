...
from .sessions import iter_jsonl, read_jsonl, session_files
...
def output_is_error(value: Any) -> bool:
    ...
    if isinstance(parsed, dict):
        return parsed.get("isError") is True or parsed.get("success") is False
    return False
...
def parse_session(
    ...
    def add_record(
        *,
        ...
        record = {
            "index": len(records) + 1,
            "id": record_id or f"record-{len(records) + 1}",
            "turn": current_turn,
            "step": step,
            "kind": kind,
            "event": event,
    ...
    for line_number, entry in entries:
            ...
            if isinstance(info, dict):
                ...
                if isinstance(last_usage, dict):
                    for record in reversed(records):
                        if record["turn"] == current_turn and record["kind"] in {
                            "assistant",
                            "reasoning",
                        }:
        ...
        if entry_type == "response_item" and payload_type in {
            ...
            completed_record = pending_calls.get(call_id)
            ...
            is_error = output_is_error(output)
            ...
            completed_record["status"] = "error" if is_error else "complete"
            ...
            if is_error:
                completed_record["error"] = "Tool result reported an error."
        ...
        if entry_type == "event_msg" and payload_type in {
            ...
            call_id = str(payload.get("call_id") or "")
            completion_record = pending_calls.get(call_id)
            if completion_record is not None:
                success = payload.get("success")
                if success is False:
                    completion_record["status"] = "error"
                    completion_record["error"] = "Tool completion event reported an error."
    ...
    omitted = max(0, all_record_count - limited)
    visible_records = records[-limited:]
    failed_tools = sum(
        1 for record in records if record["kind"] == "tool" and record["status"] == "error"
    )
    tool_calls = sum(1 for record in records if record["kind"] == "tool")
    session_id = metadata.get("session_id") or metadata.get("id") or path.stem
    git = safe_git(metadata.get("git"))
    ...
    return {
        ...
        "stats": {
            ...
            "compactions": sum(1 for record in records if record["kind"] == "compaction"),
            "tokens": latest_usage,
