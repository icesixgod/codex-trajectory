...
在 macOS 与 Windows 上，“查看轨迹”只会在携带令牌的那条 CDP 连接通过操作系统级对端认证后恢复。macOS 会解析已连接 TCP 的所有者，并要求应用是 OpenAI 团队签名的 `com.openai.codex`；Windows 会通过系统 TCP 表解析连接所有者，选择最外层已认证的 `OpenAI.Codex` 桌面祖先进程，并要求接受链路始终属于同一包系列。watcher 会拒绝 PID 复用、在 WebSocket 握手携带 viewer token 前逐连接校验，并在宿主身份消失时退出。认证失败时会安全关闭：本地 viewer URL/token 不会离开 watcher，也不注入带令牌入口；用户明确开启后，独立且有边界的直停通道仍可使用。尚未实现该对端绑定的平台继续关闭 Browser 入口。MCP 启动时会通过无窗口 supervisor 恢复原本已启用的 watcher；插件不再捆绑任何生命周期命令钩子。跨平台 launcher 在 Unix 直接执行 `uv`，在 Windows 则通过经过审阅的 GUI 子系统 shim 优先启动 `uvw.exe`，缺失时以 `CREATE_NO_WINDOW` 启动 `uv.exe`；可选 watcher 使用基础 `pythonw.exe` 并继承当前 uv 环境的导入路径，避开会重新拉起 `python.exe` 的虚拟环境转发器。因此选择 Session 不会再创建 Terminal 标签页，打包配置也不会变成 Windows 专用。该恢复过程不会替用户开启 CDP。新版运行时使用跨进程控制锁与设置修订比对替换已验证的旧 watcher，过期页面的恢复动作不会覆盖用户稍后的关闭或端口修改。心跳错误（包括全部对端认证失败）只公开固定类别，不返回异常原文或本机路径。设置、心跳和 watcher 锁仍仅作为 `CODEX_HOME/codex-trajectory` 下有大小限制的单链接普通文件读取，不会修改任务日志、草稿或附件。
