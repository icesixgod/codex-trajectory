...
在 macOS 与 Windows 上，“查看轨迹”只会在携带令牌的那条 CDP 连接通过操作系统级对端认证后恢复。macOS 会解析已连接 TCP 的所有者，并要求应用是 OpenAI 团队签名的 `com.openai.codex`；Windows 会通过系统 TCP 表解析连接所有者，并要求已认证的 `OpenAI.Codex` 包系列及进程祖先链。watcher 会发现启动插件的桌面宿主、拒绝 PID 复用、在 WebSocket 握手携带 viewer token 前逐连接校验，并在宿主身份消失时退出。认证失败时会安全关闭：本地 viewer URL/token 不会离开 watcher，也不注入带令牌入口；用户明确开启后，独立且有边界的直停通道仍可使用。尚未实现该对端绑定的平台继续关闭 Browser 入口。用户在 Codex 中一次性审核启动钩子后，插件会在会话启动时异步提前恢复原本已启用的 watcher；MCP 运行时继续作为兜底，两条路径都不会替用户开启 CDP。新版运行时使用跨进程控制锁与设置修订比对替换已验证的旧 watcher，过期页面的恢复动作不会覆盖用户稍后的关闭或端口修改。心跳错误只公开固定类别，不返回异常原文或本机路径。设置、心跳和 watcher 锁仍仅作为 `CODEX_HOME/codex-trajectory` 下有大小限制的单链接普通文件读取，不会修改任务日志、草稿或附件。
